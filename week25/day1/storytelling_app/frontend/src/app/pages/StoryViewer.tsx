...
import { getAllUsers } from '../../features/thunks/GetAllUsersThunk'; // Make sure this thunk exists and is imported
// import type { User, Contributor, Story } from '../../features/types';
import type { Contributor, Story } from '../../features/types';
...
export default function StoryViewer() {
  ...
  // State for "Add Contributor" dropdown
...
useEffect(() => {
  ...
  const handleAddContributor = async (userIdToAdd: string) => {
    ...
    try {
      // Assuming your addContributor thunk expects an object with user_id and story_id
      await dispatch(addContributor({ user_id: userIdToAdd, story_id: id })).unwrap();
      await fetchStoryContributorsList(id); // Refresh contributors list for the current story
    } catch (error) {
...
const fetchStoryContributorsList = async(story_id: string): Promise<void> => {
  if (story_id){
    try {
      ...
      setStoryContributorsList([]); // Reset to empty array on error
    }
  }
}
...
useEffect(() => {
  if (currentUser) {
    if (currentStory && (currentUser.id === currentStory.author_id)){
      // console.log("is author check: current user ID:", currentUser.id, "story author ID:", currentStory.author_id);
      setIsAuthor(true);

    } else if(
      Array.isArray(storyContributorsList) && 
      storyContributorsList.length > 0 &&
      storyContributorsList.find(c => c.user_id === currentUser.id) // Note: using user_id instead of id
    ){
      // console.log("is contributor? userid: ", currentUser.id, "in list", storyContributorsList)
...
const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {        
        ...
        const editedInfo = await dispatch(editStory({id: id, story: formData})).unwrap();
...
const handleDelete = async () => {
  ...
  if (confirmDelete) { // Only proceed if the user confirms
    ...
    setIsLoading(true);
    try {
      await dispatch(deleteStory(id!)); // Added non-null assertion for id
      dispatch(getAllStories());
      navigate('/');
    } catch (error) {
      console.log("Failed to delete story.", error); // Log the error object
      setError("Failed to delete story. Please try again."); // Set an error message for the user
    } finally {
...
// Filter users eligible for becoming contributors
const eligibleUsersForDropdown = allUsersList.filter(user => {
  // Exclude the story author
  if (currentStory && user.id === currentStory.author_id) {
    return false;
  }
  // Exclude users already in the storyContributorsList
