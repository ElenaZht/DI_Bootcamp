...
function makeAllCaps(words){
    return new Promise((res, rej) => {
        for (let w of words){
            if (typeof w !== 'string'){
                rej(`${w} is not a string`)
            }
        }
        res(words.map(w => w.toUpperCase()))
    })
}
...
function sortWords(wordsUpps){
    return new Promise((res, rej) => {
        if (wordsUpps.length > 4){
            res(wordsUpps.sort())
        } else {
            rej(`length is ${wordsUpps.length}`)
        }
    })
}
...
makeAllCaps([1, "pear", "banana"])
      ...
      .catch(error => console.log(error)) // 1 is not a string

//in this example, the catch method is executed
makeAllCaps(["apple", "pear", "banana"])
      .then((arr) => sortWords(arr))
      .then((result) => console.log(result))
      .catch(error => console.log(error)) // length is 3
